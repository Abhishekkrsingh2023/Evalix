...
function RankIcon({ rank }: { rank: number }) {
  if (rank === 1) return <Crown className="w-4 h-4 sm:w-5 sm:h-5 text-yellow-400 drop-shadow-[0_0_8px_rgba(250,204,21,0.5)]" />;
  if (rank === 2) return <Medal className="w-4 h-4 sm:w-5 sm:h-5 text-slate-300 drop-shadow-[0_0_6px_rgba(203,213,225,0.4)]" />;
  if (rank === 3) return <Medal className="w-4 h-4 sm:w-5 sm:h-5 text-amber-600 drop-shadow-[0_0_6px_rgba(217,119,6,0.4)]" />;
  return <span className="text-slate-400 font-bold text-xs sm:text-sm">#{rank}</span>;
}
  ...
  return (
    <div className="space-y-6 min-w-0 w-full max-w-full">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2 border-b border-slate-800">
        ...
        {/* Live Toggle Switch */}
        <div className="flex items-center gap-3 bg-slate-800/80 p-1.5 px-3 rounded-2xl border border-slate-700 shrink-0 self-start sm:self-auto">
      ...
      {!isLive ? (
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-slate-900/60 p-2 rounded-2xl border border-slate-800">
            <div className="flex items-center gap-1.5 p-1 bg-slate-800/80 rounded-xl border border-slate-700/50 overflow-x-auto">
              <button
          ...
          {data && (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <Card className="p-3 sm:p-4 text-center bg-slate-900/50">
          ...
          <Card className="overflow-hidden p-0 border border-slate-800 bg-slate-900/60 min-w-0 w-full">
            {loading && !data ? (
              ...
              <>
                ...
                {top3.length > 0 && (
                  <div className="p-3 sm:p-5 border-b border-slate-800 bg-gradient-to-b from-slate-800/40 to-transparent">
                    ...
                    <div className="grid grid-cols-3 gap-2 sm:gap-4">
                      {top3.map((entry) => {
                        ...
                        return (
                          <div
                            key={entry.team_id}
                            className={`text-center p-2.5 sm:p-4 rounded-xl border flex flex-col items-center justify-between min-w-0 transition-all ${
                              entry.rank === 1
                          ...
                          >
                            ...
                            <p className="font-semibold text-slate-100 text-xs sm:text-sm mt-1 truncate w-full" title={entry.team_name}>
                              {entry.team_name}
                            </p>
                      ...
                      <div key={entry.team_id} className="p-3.5 sm:p-4 space-y-2.5 hover:bg-slate-800/20 transition-colors">
                        ...
                        <div className="flex items-start justify-between gap-2.5">
                          ...
                          <div className="text-right shrink-0">
                            <div className={`font-bold text-base sm:text-lg leading-tight ${
                              activeTab === 'final' ? 'text-amber-400' : 'text-violet-400'
                            }`}>
                              {activeScoreVal !== null && activeScoreVal !== undefined ? activeScoreVal.toFixed(1) : '—'}
                            </div>
                            <span className="text-[9px] text-slate-500 uppercase tracking-wider font-semibold">
                              {activeTab === 'final' ? 'overall' : activeTab === '1' ? 'day 1' : 'day 2'}
                            </span>
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-3 gap-1.5 py-1.5 px-2.5 rounded-lg bg-slate-900/60 border border-slate-800/70 text-xs">
                          ...
                          <div className={activeTab === 'final' ? 'font-bold text-amber-300' : ''}>
                            <span className="text-slate-500 block text-[10px] uppercase">Final Avg</span>
                            <span className="font-medium text-xs">
          ...
          <Card className="border border-slate-800 bg-slate-900/30 p-4 sm:p-5">
